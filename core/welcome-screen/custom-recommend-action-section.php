 ...
 * @access public
 */
class Transcend_Customize_Section_Recommend extends WP_Customize_Section {
	/**
	 ...
	 * @var    string
	 */
	public $type = 'transcend-recomended-section';
	/**
	...
	public function call_plugin_api( $slug ) {
		...
		if ( false === ( $call_api = get_transient( 'transcend_plugin_information_transient_' . $slug ) ) ) {
			...
			set_transient( 'transcend_plugin_information_transient_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $transcend_required_actions as $key => $transcend_required_action ) {
			if ( @$transcend_show_required_actions[ $transcend_required_action['id'] ] === false ) {
				continue;
			}
			if ( $transcend_required_action['check'] ) {
				continue;
			}

			$transcend_required_action['index'] = $key + 1;

			if ( isset($transcend_required_action['plugin_slug']) ) {
				$active = $this->check_active( $transcend_required_action['plugin_slug'] );
				$transcend_required_action['url']    = $this->create_action_link( $active['needs'], $transcend_required_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$transcend_required_action['class'] = 'active';
				}else{
					$transcend_required_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$transcend_required_action['button_class'] = 'install-now button';
						$transcend_required_action['button_label'] = __( 'Install', 'transcend' );
						break;
					case 'activate':
						$transcend_required_action['button_class'] = 'activate-now button button-primary';
						$transcend_required_action['button_label'] = __( 'Activate', 'transcend' );
						break;
					case 'deactivate':
						$transcend_required_action['button_class'] = 'deactivate-now button';
						$transcend_required_action['button_label'] = __( 'Deactivate', 'transcend' );
						break;
				}

			}
			$formatted_array[] = $transcend_required_action;
		}
		...
		foreach ( $transcend_recommended_plugins as $slug => $plugin_opt ) {
			...
			$transcend_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$transcend_recommended_plugin['class'] = 'active';
			}else{
				$transcend_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$transcend_recommended_plugin['button_class'] = 'install-now button';
					$transcend_recommended_plugin['button_label'] = __( 'Install', 'transcend' );
					break;
				case 'activate':
					$transcend_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$transcend_recommended_plugin['button_label'] = __( 'Activate', 'transcend' );
					break;
				case 'deactivate':
					$transcend_recommended_plugin['button_class'] = 'deactivate-now button';
					$transcend_recommended_plugin['button_label'] = __( 'Deactivate', 'transcend' );
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$transcend_recommended_plugin['id'] = $slug;
			$transcend_recommended_plugin['plugin_slug'] = $slug;
			$transcend_recommended_plugin['description'] = $info->short_description;
			$transcend_recommended_plugin['title'] = $transcend_recommended_plugin['button_label'].': '.$info->name;

			$customize_plugins[] = $transcend_recommended_plugin;
		...
		$json['total_actions'] = count($transcend_required_actions);
		$json['social_text'] = $this->social_text;
		$json['plugin_text'] = $this->plugin_text;
		$json['facebook'] = $this->facebook;
		$json['facebook_text'] = __( 'Facebook', 'transcend' );
		$json['twitter'] = $this->twitter;
		$json['twitter_text'] = __( 'Twitter', 'transcend' );
		$json['wp_review'] = $this->wp_review;
		$json['wp_review_text'] = __( 'Review this theme on w.org', 'transcend' );
		if ( $this->wp_review ) {
	...
	protected function render_template() { ?>
		<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
			...
			<h3 class="accordion-section-title">
				...
				<# if( data.required_actions.length > 0 ){ #>
					<span class="transcend-actions-count">
			...
			<div class="recomended-actions_container" id="plugin-filter">
						...
						<div class="epsilon-recommeded-actions-container epsilon-required-actions" data-index="{{ data.required_actions[action].index }}">
							<# if( !data.required_actions[action].check ){ #>
								<div class="epsilon-recommeded-actions">
									...
									<span data-action="dismiss" class="dashicons dashicons-visibility transcend-dismiss-required-action" id="{{ data.required_actions[action].id }}"></span>
						...
						<div class="epsilon-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
							<# if( !data.recommended_plugins[action].check ){ #>
								<div class="epsilon-recommeded-actions">
									...
									<span data-action="dismiss" class="dashicons dashicons-visibility transcend-recommended-plugin-button" id="{{ data.recommended_plugins[action].id }}"></span>
				...
				<# if( data.required_actions.length == 0 && data.recommended_plugins.length == 0 ){ #>
					<p class="succes">
						...
						<# } #>
						<# if( data.wp_review ){ #>
							<a href="https://wordpress.org/support/theme/{{ data.theme_slug }}/reviews/#new-post" class="button button-primary transcend-wordpress" target="_blank"><span class="dashicons dashicons-wordpress"></span>{{ data.wp_review_text }}</a>
						<# } #>
					</p>
				<# }else{ #>
					<p class="succes hide">
