...
class Transcend_Welcome {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'transcend_welcome_register_menu' ) );

		/* activation notice */
		add_action( 'load-themes.php', array( $this, 'transcend_activation_admin_notice' ) );
		...
		add_action( 'admin_init', array( $this, 'transcend_set_pages' ) );
	}

	public function transcend_set_pages() {
		if ( ! empty( $_GET ) ) {
			/**
			 * Check action
			 */
	...
	public function transcend_activate_plugin() {
		if ( ! empty( $_GET ) ) {
			/**
			 * Check action
			 */
	...
	public function transcend_deactivate_plugin() {
		if ( ! empty( $_GET ) ) {
			/**
			 * Check action
			 */
	 ...
	 * @since 1.8.2.4
	 */
	public function transcend_welcome_register_menu() {
		$action_count = $this->count_actions();
		$title        = $action_count > 0 ? 'About Transcend <span class="badge-action-count">' . esc_html( $action_count ) . '</span>' : 'About Transcend';

		add_theme_page( 'About Transcend', $title, 'edit_theme_options', 'cpotheme-welcome', array(
			$this,
			'transcend_welcome_screen'
		) );
	}
	 ...
	 * @since 1.8.2.4
	 */
	public function transcend_activation_admin_notice() {
		global $pagenow;

		if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
			add_action( 'admin_notices', array( $this, 'transcend_welcome_admin_notice' ), 99 );
		}
	}
	 ...
	 * @since 1.8.2.4
	 */
	public function transcend_welcome_admin_notice() {
		?>
		<div class="updated notice is-dismissible">
			<p><?php echo sprintf( esc_html__( 'Welcome! Thank you for choosing Transcend! To fully take advantage of the best our theme can offer please make sure you visit our %swelcome page%s.', 'transcend' ), '<a href="' . esc_url( admin_url( 'themes.php?page=cpotheme-welcome' ) ) . '">', '</a>' ); ?></p>
			<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=cpotheme-welcome' ) ); ?>" class="button"
			      style="text-decoration: none;"><?php _e( 'Get started with Transcend', 'transcend' ); ?></a></p>
		</div>
		<?php
	}
	...
	public function count_actions() {
		...
		$transcend_show_required_actions = get_option( 'transcend_show_required_actions' );
		if ( ! $transcend_show_required_actions ) {
			$transcend_show_required_actions = array();
		}

		$i = 0;
		foreach ( $transcend_required_actions as $action ) {
			$true      = false;
			$dismissed = false;
			...
			if ( ! empty( $transcend_show_required_actions ) && isset( $transcend_show_required_actions[ $action['id'] ] ) && ! $transcend_show_required_actions[ $action['id'] ] ) {
				$true = false;
			}
	 ...
	 * @since 1.8.2.4
	 */
	public function transcend_welcome_screen() {
		require_once( ABSPATH . 'wp-load.php' );
		require_once( ABSPATH . 'wp-admin/admin.php' );
		require_once( ABSPATH . 'wp-admin/admin-header.php' );

		$transcend      = wp_get_theme();
		...
		<div class="wrap about-wrap epsilon-wrap">
			...
			<h1><?php echo __( 'Welcome to Transcend! - Version ', 'transcend' ) . $transcend['Version']; ?></h1>

			<div
				class="about-text"><?php echo esc_html__( 'Transcend is now installed and ready to use! Get ready to build something beautiful. We hope you enjoy it! We want to make sure you have the best experience using Transcend and that is why we gathered here all the necessary information for you. We hope you will enjoy using Transcend, as much as we enjoy creating great products.', 'transcend' ); ?></div>
			...
			<h2 class="nav-tab-wrapper wp-clearfix">
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=getting_started' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'getting_started' ? 'nav-tab-active' : ''; ?>"><?php echo esc_html__( 'Getting Started', 'transcend' ); ?></a>
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=recommended_actions' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'recommended_actions' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Recommended Actions', 'transcend' ); ?>
				...
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=features' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'features' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Lite vs PRO', 'transcend' ); ?></a>
			</h2>
