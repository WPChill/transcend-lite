...
if ( ! class_exists( 'MT_Notify_System' ) ) {
	...
	class MT_Notify_System {
		...
		public static function version_check( $ver ) {
			...
			return version_compare( $transcend['Version'], $ver, '>=' );
		}
		...
		public static function widget_importer_exporter_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: Widget Importer Exporter Plugin', 'transcend' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Activate: Widget Importer Exporter Plugin', 'transcend' );
			}
		...
		public static function wordpress_importer_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: WordPress Importer', 'transcend' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Activate: WordPress Importer', 'transcend' );
			}
		...
		public static function wordpress_importer_description() {
			$installed = self::check_plugin_is_installed( 'wordpress-importer' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress Importer to create the demo content.', 'transcend' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Importer to create the demo content.', 'transcend' );
			}
		...
		public static function widget_importer_exporter_description() {
			$installed = self::check_plugin_is_installed( 'widget-importer-exporter' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress widget importer to create the demo content', 'transcend' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Widget Importer to create the demo content.', 'transcend' );
			}
